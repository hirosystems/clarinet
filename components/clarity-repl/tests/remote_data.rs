...
const COUNTER_CONTRACT: &str = "STJCAB2T9TR2EJM7YS4DM2CGBBVTF7BV237Y8KNV.counter";
...
fn it_handles_not_found_contract() {
    ...
    let snippet = format!("(contract-call? '{} get-count)", COUNTER_CONTRACT);
...
fn it_can_fork_state() {
    let mut session = init_session(57000);
    let snippet_get_count = format!("(contract-call? '{} get-count)", COUNTER_CONTRACT);
    let result = eval_snippet(&mut session, &snippet_get_count);
    assert_eq!(result, Value::UInt(1));

    let snippet = format!("(contract-call? '{} increment)", COUNTER_CONTRACT);
