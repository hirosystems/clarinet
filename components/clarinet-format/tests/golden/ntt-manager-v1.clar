...
(define-public (admin-register-transceiver (transceiver <transceiver-trait>))
  (let ((contract (contract-of transceiver))
...
(define-public (send-tokens (transceiver <transceiver-trait>) (amount uint) (fee uint) (to-chain (buff 16)) (to (buff 32)))
  (begin
    (try! (check-transceiver transceiver))
    ;; Take tokens from user and lock them in this contract
    ;; TODO: Add memo?
    (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer amount tx-sender (get-contract-principal) none))
    (contract-call? transceiver send-token-transfer amount 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token to to-chain fee)))

;; @desc Lock tokens and send cross-chain message via specified transceiver
(define-public (receive-tokens (transceiver <transceiver-trait>) (bytes (buff 8192)))
  (let ((check (try! (check-transceiver transceiver)))
    ...
    (asserts! (map-insert consumed-messages (get uid result) true) ERR_RECV_ALREADY_USED)
    (match (get recipient result)
      ;; Registered, unlock and send to account
      ;; TODO: Add memo?
      recipient (try! (as-contract (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer amount tx-sender recipient none)))
      ...
      (let ((idx {
        ...
        (map-set tokens-pending idx (+ amount (default-to u0 (get-tokens-pending idx))))))
    (ok true)))
...
(define-read-only (get-tokens-locked (account principal))
  (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance (get-contract-principal)))
