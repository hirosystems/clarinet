...
(define-private (bool-list-of-len
		(n uint)
	)
...
;; Returns (err ERR-OUT-OF-BOUNDS) if we read past the end of txbuff.
(define-read-only (read-varslice
		(old-ctx {
				txbuff: (buff 4096),
				index: uint
			}
...
;; Returns (err ERR-OUT-OF-BOUNDS) if we read past the end of txbuff.
(define-read-only (read-hashslice
		(old-ctx {
				txbuff: (buff 4096),
				index: uint
			}
					...
					txins: (list 8
						{
							...
							scriptSig: (buff 256),
							;; just big enough to hold a 2-of-3 multisig script
							sequence: uint
						}
					)
				}
				...
				(ok
					{
						witnesses: (unwrap-panic
							(as-max-len? (append (get witnesses state) (get items parsed-items)) u8)
						),
...
(define-read-only (parse-wtx
	...
	(let (
		...
		(parsed-witnesses
			(try! (read-witnesses (get ctx parsed-txouts) (len (get txins parsed-txins))))
		)
								...
								(sha256
									(concat (unwrap-panic (slice? tx u0 u4))
										(concat
											...
											(unwrap-panic
												(slice? tx (get index (get ctx parsed-witnesses)) (len tx))
											)
										)
									)
								)
...
;; })
(define-read-only (parse-block-header
		(headerbuff (buff 80))
	)
	(let (
		(ctx {
...
(define-read-only (get-bc-h-hash
		(bh uint)
	)
	(if DEBUG-MODE
...
(define-read-only (get-reversed-txid
		(tx (buff 4096))
	)
	(sha256 (sha256 tx))
)
...
(define-read-only (is-commitment-pattern
		(scriptPubKey (buff 128))
	)
