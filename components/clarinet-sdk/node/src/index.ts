...
type Options = { trackCosts: boolean; trackCoverage: boolean; trackPerformance?: boolean; performanceCostField?: string };

export async function getSDK(options?: Options): Promise<Simnet> {
  const module = await wasmModule;
  let sdkOptions = new SDKOptions(!!options?.trackCosts, !!options?.trackCoverage, options?.trackPerformance, options?.performanceCostField);
...
function memoizedInit() {
  ...
  return async (
    ...
    noCache = false,
    options?: { trackCosts: boolean; trackCoverage: boolean; trackPerformance?: boolean; performanceCostField?: string },
  ) => {
    if (noCache || !simnet) {
