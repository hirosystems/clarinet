...
fn test_bitcoin_chainhook_through_reorg() {
    let (observer_commands_tx, observer_commands_rx) = channel();
    let (block_pre_processor_in_tx, _block_pre_processor_in_rx) = crossbeam_channel::unbounded();
    let (_block_pre_processor_out_tx, block_pre_processor_out_rx) = crossbeam_channel::unbounded();
