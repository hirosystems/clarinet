...
impl GraphData {
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&ModuleSpecifier, &ModuleEntry)> {
        self.modules.iter()
    }
    ...
    pub fn graph_segment(&self, roots: &[(ModuleSpecifier, ModuleKind)]) -> Option<Self> {
    ...
    pub fn set_type_checked(&mut self, roots: &[(ModuleSpecifier, ModuleKind)], lib: TsTypeLib) {
        let specifiers: Vec<ModuleSpecifier> = match self.walk(roots, true, true, true) {
    ...
    pub fn is_type_checked(
        ...
        roots.iter().all(|(r, _)| {
            ...
            match self.modules.get(&found) {
                Some(ModuleEntry::Module { checked_libs, .. }) => checked_libs.contains(lib),
                _ => false,
            }
        })
    }
    ...
    pub fn follow_redirect(&self, specifier: &ModuleSpecifier) -> ModuleSpecifier {
    ...
    pub fn get<'a>(&'a self, specifier: &ModuleSpecifier) -> Option<&'a ModuleEntry> {
    ...
    // memory consumption
    pub fn add_cjs_esm_translation(&mut self, specifier: &ModuleSpecifier, source: String) {
        let prev = self
