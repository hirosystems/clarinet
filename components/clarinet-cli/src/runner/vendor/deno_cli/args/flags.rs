        ...
        .arg(
            Arg::new("options-line-width")
                ...
                .validator(|val: &str| match val.parse::<NonZeroUsize>() {
                    Ok(_) => Ok(()),
                    Err(_) => Err("options-line-width should be a non zero integer".to_string()),
                }),
        )
        .arg(
            Arg::new("options-indent-width")
                ...
                .validator(|val: &str| match val.parse::<NonZeroUsize>() {
                    Ok(_) => Ok(()),
                    Err(_) => Err("options-indent-width should be a non zero integer".to_string()),
                }),
        )
        .arg(
...
fn task_parse(flags: &mut Flags, matches: &clap::ArgMatches, raw_args: &[String]) {
...
fn compile_args_without_no_check_parse(flags: &mut Flags, matches: &clap::ArgMatches) {
    import_map_arg_parse(flags, matches);
    no_remote_arg_parse(flags, matches);
    config_args_parse(flags, matches);
    reload_arg_parse(flags, matches);
...
fn permission_args_parse(flags: &mut Flags, matches: &clap::ArgMatches) {
    ...
    if let Some(net_wl) = matches.values_of("allow-net") {
        let net_allowlist: Vec<String> =
            flags_allow_net::parse(net_wl.map(ToString::to_string).collect()).unwrap();
...
fn unsafely_ignore_certificate_errors_parse(flags: &mut Flags, matches: &clap::ArgMatches) {
...
fn enable_testing_features_arg_parse(flags: &mut Flags, matches: &clap::ArgMatches) {
