...
pub static STATIC_ASSETS: Lazy<HashMap<&'static str, &'static str>> = Lazy::new(|| {
    (&[
        (
...
fn get_maybe_hash(maybe_source: Option<&str>, hash_data: &[Vec<u8>]) -> Option<String> {
...
fn normalize_specifier(specifier: &str) -> Result<ModuleSpecifier, AnyError> {
    resolve_url_or_path(&specifier.replace(".d.ts.d.ts", ".d.ts")).map_err(|err| err.into())
}
...
fn op_load(state: &mut OpState, args: Value) -> Result<Value, AnyError> {
    ...
    // handle the request for that module here.
        ...
        let specifier =
            if let Some(remapped_specifier) = state.remapped_specifiers.get(&v.specifier) {
    ...
    Ok(json!({ "data": data, "version": hash, "scriptKind": as_ts_script_kind(&media_type) }))
}
    ...
    for specifier in &args.specifiers {
        if specifier.starts_with("asset:///") {
            ...
            let resolved_dep = match graph_data.get(&referrer) {
                ...
                Some(ModuleEntry::Configuration { dependencies }) => dependencies.get(specifier),
                _ => None,
            };
