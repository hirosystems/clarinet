...
pub async fn format(config: &CliOptions, fmt_flags: FmtFlags) -> Result<(), AnyError> {
    ...
    let fmt_predicate = |path: &Path| {
        is_supported_ext_fmt(path) && !contains_git(path) && !contains_node_modules(path)
    };

    let resolver = |changed: Option<Vec<PathBuf>>| {
        let files_changed = changed.is_some();

        let result = collect_files(&include_files, &exclude_files, fmt_predicate).map(|files| {
        ...
        async move {
            if files_changed && matches!(result, Ok((ref files, _)) if files.is_empty()) {
    ...
    let operation = |(paths, fmt_options): (Vec<PathBuf>, FmtOptionsConfig)| async move {
        ...
        if check {
            ...
            format_source_files(paths, fmt_options, incremental_cache.clone()).await?;
        }
    ...
    if config.watch_paths().is_some() {
        ...
        let files =
            collect_files(&include_files, &exclude_files, fmt_predicate).and_then(|files| {
                if files.is_empty() {
...
pub fn format_stdin(fmt_flags: FmtFlags, fmt_options: FmtOptionsConfig) -> Result<(), AnyError> {
...
fn resolve_fmt_options(fmt_flags: &FmtFlags, options: FmtOptionsConfig) -> FmtOptionsConfig {
