...
impl VendorTestBuilder {
    ...
    pub async fn build(&mut self) -> Result<VendorOutput, AnyError> {
        ...
        let graph = build_test_graph(roots, self.original_import_map.clone(), loader.clone()).await;
        super::build::build(
            graph,
            &output_dir,
...
async fn build_test_graph(
    ...
    let resolver = original_import_map.map(|m| ImportMapResolver::new(Arc::new(m)));
