...
fn maybe_update_config_file(output_dir: &Path, ps: &ProcState) -> bool {
    ...
    let result = update_config_file(
        ...
        &ModuleSpecifier::from_file_path(output_dir.join("import_map.json")).unwrap(),
        &fmt_config.options,
    );
    match result {
        Ok(()) => true,
...
fn update_config_file(
    ...
    let relative_text = match relative_specifier(config_specifier, import_map_specifier) {
        Some(text) => text,
        None => return Ok(()), // ignore
    };
    if let Some(new_text) = update_config_text(&config_text, &relative_text, fmt_options) {
...
fn update_config_text(
    ...
    let ast = jsonc_parser::parse_to_ast(text, &Default::default(), &Default::default()).ok()?;
