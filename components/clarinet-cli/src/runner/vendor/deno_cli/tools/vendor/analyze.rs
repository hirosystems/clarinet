...
mod test {
    ...
    fn has_default_when_named_export_alias() {
        let parsed_source = parse_module("export {test as default} from './test.ts';");
        assert!(has_default_export(&parsed_source));
    }

    #[test]
    fn not_has_default_when_named_export_not_exported() {
        let parsed_source = parse_module("export {default as test} from './test.ts';");
        assert!(!has_default_export(&parsed_source));
    }
    ...
    fn not_has_default_when_not() {
        let parsed_source = parse_module(
            "export {test} from './test.ts'; export class Test{} export * from './test';",
        );
        assert!(!has_default_export(&parsed_source));
    }
