...
fn generate_coverage_report(
    ...
    for function in &script_coverage.functions {
        ...
        let dest_line_index = text_lines.line_index(
            text_lines.byte_index_from_char_index(function.ranges[0].start_char_offset),
        );
    ...
    for (block_number, function) in script_coverage.functions.iter().enumerate() {
        ...
        for (branch_number, range) in function.ranges[1..].iter().enumerate() {
            let source_line_index = text_lines
                .line_index(text_lines.byte_index_from_char_index(range.start_char_offset));
            let line_index = if let Some(source_map) = maybe_source_map.as_ref() {
                source_map
                    .tokens()
...
pub async fn cover_files(flags: Flags, coverage_flags: CoverageFlags) -> Result<(), AnyError> {
    ...
    let script_coverages = collect_coverages(coverage_flags.files, coverage_flags.ignore)?;
    ...
    for script_coverage in script_coverages {
        let module_specifier = deno_core::resolve_url_or_path(&script_coverage.url)?;
        ...
        let maybe_file = if module_specifier.scheme() == "file" {
            ...
            ps.file_fetcher
                ...
                .with_context(|| format!("Failed to fetch \"{}\" from cache.", module_specifier))?
        };
