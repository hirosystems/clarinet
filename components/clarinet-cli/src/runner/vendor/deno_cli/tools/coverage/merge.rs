...
pub fn merge_processes(mut processes: Vec<ProcessCoverage>) -> Option<ProcessCoverage> {
    ...
    let mut url_to_scripts: BTreeMap<String, Vec<ScriptCoverage>> = BTreeMap::new();
...
pub fn merge_scripts(mut scripts: Vec<ScriptCoverage>) -> Option<ScriptCoverage> {
    if scripts.len() <= 1 {
        return scripts.pop();
    }
    let (script_id, url) = {
    ...
    let mut range_to_funcs: BTreeMap<CharRange, Vec<FunctionCoverage>> = BTreeMap::new();
...
pub fn merge_functions(mut funcs: Vec<FunctionCoverage>) -> Option<FunctionCoverage> {
    if funcs.len() <= 1 {
        return funcs.pop();
    }
...
fn merge_range_tree_children<'a>(
    ...
    let mut child_forests: Vec<Peekable<::std::vec::IntoIter<&'a mut RangeTree<'a>>>> =
        child_forests
            .into_iter()
