...
pub async fn run_benchmarks_with_watch(
    ...
    let operation = |modules_to_reload: Vec<(ModuleSpecifier, ModuleKind)>| {
        ...
        async move {
            let specifiers = collect_specifiers(include.clone(), &ignore, is_supported_bench_path)?
                .iter()
            ...
            bench_specifiers(ps, permissions.clone(), specifiers, specifier_options).await?;
