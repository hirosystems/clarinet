...
use super::args::CliOptions;
use super::args::DenoSubcommand;
...
use super::compat;
use super::compat::NodeEsmResolver;
use super::deno_dir;
use super::emit;
...
impl ProcState {
    ...
    pub async fn from_options(options: Arc<CliOptions>) -> Result<Self, AnyError> {
    ...
    async fn build_with_sender(
        ...
        let maybe_import_map_specifier = cli_options.resolve_import_map_specifier()?;
        ...
        let maybe_import_map = if let Some(import_map_specifier) = maybe_import_map_specifier {
            ...
            let import_map = import_map_from_text(&import_map_specifier, &file.source)?;
        ...
        // created below
        let node_resolver =
            NodeEsmResolver::new(maybe_import_map.clone().map(ImportMapResolver::new));
        let maybe_import_map_resolver = maybe_import_map.clone().map(ImportMapResolver::new);
        let maybe_jsx_resolver = cli_options
            .to_maybe_jsx_import_source_module()
            .map(|im| JsxResolver::new(im, maybe_import_map_resolver.clone()));
        let maybe_resolver: Option<Arc<dyn deno_graph::source::Resolver + Send + Sync>> =
    ...
    pub async fn prepare_module_load(
        ...
        let roots = roots
            ...
            .map(|r| {
                if let Some(resolver) = &maybe_resolver {
                    let response = resolver.resolve(r.as_str(), &Url::parse("unused:").unwrap());
        ...
        impl Loader for ProcStateLoader<'_> {
            ...
            fn load(&mut self, specifier: &ModuleSpecifier, is_dynamic: bool) -> LoadFuture {
                ...
                match graph_data.get(&found_specifier) {
                    Some(_) if !self.reload => Box::pin(futures::future::ready(Err(anyhow!("")))),
        ...
        if self.options.type_check_mode() != TypeCheckMode::None {
            ...
            let check_cache = TypeCheckCache::new(&self.dir.type_checking_cache_db_file_path());
            let graph_data = self.graph_data.clone();
            let check_result = emit::check(&roots, graph_data, &check_cache, options)?;
    ...
    pub fn resolve(&self, specifier: &str, referrer: &str) -> Result<ModuleSpecifier, AnyError> {
    ...
    pub async fn create_graph(
        ...
        let maybe_locker = as_maybe_locker(self.lockfile.clone());
        let maybe_import_map_resolver = self.maybe_import_map.clone().map(ImportMapResolver::new);
        let maybe_imports = self.options.to_maybe_imports()?;
...
pub fn import_map_from_text(specifier: &Url, json_text: &str) -> Result<ImportMap, AnyError> {
    debug_assert!(
