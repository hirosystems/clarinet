...
fn get_tsc_roots(graph_data: &GraphData, check_js: bool) -> Vec<(ModuleSpecifier, MediaType)> {
    graph_data
        .entries()
        .into_iter()
...
pub fn emit_parsed_source(
    ...
    let source_hash = get_source_hash(parsed_source.text_info().text_str(), emit_config_hash);
...
/// be used to tell
fn get_check_hash(graph_data: &GraphData, options: &CheckOptions) -> CheckHashResult {
