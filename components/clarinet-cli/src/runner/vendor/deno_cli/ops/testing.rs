...
use super::super::super::deno_runtime::permissions::create_child_permissions;
use super::super::super::deno_runtime::permissions::ChildPermissionsArg;
...
#[op]
pub fn op_restore_test_permissions(state: &mut OpState, token: Uuid) -> Result<(), AnyError> {
...
#[op]
fn op_register_test(state: &mut OpState, info: TestInfo) -> Result<TestRegisterResult, AnyError> {
...
#[op]
fn op_dispatch_test_event(state: &mut OpState, event: TestEvent) -> Result<(), AnyError> {
    let mut sender = state.borrow::<TestEventSender>().clone();
