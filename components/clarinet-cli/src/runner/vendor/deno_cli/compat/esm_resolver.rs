...
impl Resolver for NodeEsmResolver {
    fn resolve(&self, specifier: &str, referrer: &ModuleSpecifier) -> ResolveResponse {
        ...
        let node_resolution = node_resolve(specifier, referrer.as_str(), &current_dir);
...
fn package_imports_resolve(
    ...
    if package_config.exists {
        package_json_url = Some(Url::from_file_path(package_config.pjsonpath).unwrap());
...
fn throw_invalid_subpath(
    ...
    errors::err_invalid_module_specifier(&subpath, &reason, Some(to_file_path_string(base)))
}
...
fn resolve_package_target_string(
    ...
    let invalid_segment_re = Regex::new(r"(^|\|/)(..?|node_modules)(\|/|$)").expect("bad regex");
    ...
    if pattern {
        let replaced =
            pattern_re.replace(resolved.as_str(), |_caps: &regex::Captures| subpath.clone());
...
fn package_exports_resolve(
    ...
    let exports_map = if is_conditional_exports_main_sugar(exports, &package_json_url, base)? {
        let mut map = Map::new();
...
fn package_resolve(
    ...
    let package_config = get_package_scope_config(base)?;
    if package_config.exists {
        let package_json_url = Url::from_file_path(&package_config.pjsonpath).unwrap();
    ...
    loop {
        ...
        if !is_dir {
            ...
            package_json_url =
                package_json_url.join(&format!("{}{}/package.json", prefix, package_name))?;
            package_json_path = to_file_path(&package_json_url);
            if package_json_path.to_str().unwrap().len() == last_path.to_str().unwrap().len() {
                break;
            } else {
                continue;
            }
        ...
        // Package match.
        let package_config = get_package_config(package_json_path.clone(), specifier, Some(base))?;
...
pub fn check_if_should_use_esm_loader(main_module: &ModuleSpecifier) -> Result<bool, AnyError> {
...
fn get_package_config(
    ...
    let package_json: Value = serde_json::from_str(&source).map_err(|err| {
        let base_msg = maybe_base
            .map(|base| format!("\"{}\" from {}", specifier, to_file_path(base).display()));
...
fn get_package_scope_config(resolved: &ModuleSpecifier) -> Result<PackageConfig, AnyError> {
    ...
    loop {
        ...
        let package_config =
            get_package_config(to_file_path(&package_json_url), resolved.as_str(), None)?;
...
mod tests {
    ...
    fn basic() {
        ...
        let expected = Url::from_file_path(cwd.join("node_modules/foo/index.js")).unwrap();
        ...
        .unwrap();
        let expected =
            Url::parse("data:application/javascript,console.log(\"Hello%20Deno\");").unwrap();
    ...
    fn package_subpath() {
        ...
        let expected = Url::from_file_path(cwd.join("node_modules/foo/server.js")).unwrap();
        matches!(actual, ResolveResponse::CommonJs(_));
        assert_eq!(actual.to_result().unwrap(), expected);
    }
    ...
    fn basic_deps() {
        ...
        let main = Url::from_file_path(cwd.join("main.js")).unwrap();
        let actual = node_resolve("foo", main.as_str(), &cwd).unwrap();
        let foo_js = Url::from_file_path(cwd.join("node_modules/foo/foo.js")).unwrap();
        assert!(matches!(actual, ResolveResponse::Esm(_)));
        assert_eq!(actual.to_result().unwrap(), foo_js);

        let actual = node_resolve("bar", foo_js.as_str(), &cwd).unwrap();

        let bar_js = Url::from_file_path(cwd.join("node_modules/bar/bar.js")).unwrap();
        assert!(matches!(actual, ResolveResponse::Esm(_)));
    ...
    fn conditional_exports() {
        ...
        let expected =
            Url::from_file_path(cwd.join("node_modules/imports_exports/import_export.js")).unwrap();
