...
use clarinet_core::{CheckCheckerSettings, DEFAULT_EPOCH};
...
use crate::annotation::{Annotation, AnnotationKind, WarningKind};
use crate::ast_visitor::{traverse, ASTVisitor, TypedVar};
use crate::{AnalysisPass, AnalysisResult};
...
impl<'a> CheckChecker<'a> {
    fn new(annotations: &'a Vec<Annotation>, settings: CheckCheckerSettings) -> CheckChecker<'a> {
        Self {
            settings,
...
impl AnalysisPass for CheckChecker<'_> {
    fn run_pass(
        ...
        settings: &crate::AnalysisSettings,
    ) -> AnalysisResult {
