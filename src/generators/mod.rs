...
pub fn get_changes_for_new_contract(manifest_path: PathBuf, contract_name: String, source: Option<String>, include_test: bool, deps: Vec<String>) -> Vec<Changes> {
    let mut command = GetChangesForNewContract::new(manifest_path, contract_name, source);
    command.run(include_test, deps)
}

pub fn get_changes_for_new_link(manifest_path: PathBuf, contract_id: String, _source: Option<String>) -> Vec<Changes> {
