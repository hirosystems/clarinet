...
use clarity_repl::repl::{OutputMode, Session};
...
impl EventObserverConfig {
    pub fn new(
        ...
        let session = match load_session(manifest_path.clone(), false, Network::Devnet, OutputMode::Console) { //Todo: get output mode as param
            Ok(session) => session,
            Err(e) => {
                println!("{}", e);
                std::process::exit(1);
